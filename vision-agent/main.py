...
async def create_agent(**kwargs) -> Agent:
    ...
    return Agent(
        edge=getstream.Edge(),
        llm=gemini.Realtime(model=model_name),
...
async def join_call(agent: Agent, call_type: str, call_id: str, **kwargs) -> None:
    ...
    if call_type != "default":
        try:
            ...
            print(f"[agent] Warning: go_live failed for call type {call_type}: {e}")
    else:
        print("[agent] Skipping go_live for default call type (backstage is not enabled).")
