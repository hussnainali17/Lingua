...
export default function LessonScreen() {
  ...
  const startCall = useCallback(async () => {
    ...
    if (callRef.current || clientRef.current) return;
    ...
    try {
      ...
      const language = selectedLanguage?.code ?? lesson.id.split("-")[0];
      const introMessage =
        lesson.teacherPrompt.prompts[0] ?? "Welcome to this lesson.";
      ...
      try {
        await streamCall.update({
          custom: {
            ...
            phrases: lesson.phrases.map((item) => item.text),
            topics: lesson.teacherPrompt.prompts,
            system_prompt: lesson.teacherPrompt.instructions,
      ...
      const message =
        err instanceof Error
          ? err.message
          : "We could not connect to the lesson right now.";
  ...
  }, [ensureCallPermissions, getToken, lesson, selectedLanguage, user]);
  ...
  useEffect(() => {
    ...
    posthog.capture("lesson_started", {
      lesson_id: lesson.id,
      language: selectedLanguage?.code ?? lesson.id.split("-")[0],
...
function getIntroMessage(lesson: Lesson): string {
  const intro = lesson.teacherPrompt.prompts[0] ?? "Welcome to this lesson";
  return intro.replace(/!+$/, "").trim();
}
